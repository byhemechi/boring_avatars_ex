defmodule BoringAvatar.Variant.Beam do
  @size 36

  import BoringAvatar.Utilities
  import BoringAvatar.SVG

  @behaviour BoringAvatar

  def generate_data(name, colours) do
    num_from_name = hash_code(name)
    range = length(colours)

    wrapper_colour = get_random_colour(num_from_name, colours, range)
    pre_translate_x = get_unit(num_from_name, 10, 1)

    wrapper_translate_x =
      if(pre_translate_x < 5, do: pre_translate_x + @size / 9, else: pre_translate_x)

    pre_translate_y = get_unit(num_from_name, 10, 2)

    wrapper_translate_y =
      if(pre_translate_y < 5, do: pre_translate_y + @size / 9, else: pre_translate_y)

    %{
      wrapper_colour: wrapper_colour,
      face_colour: get_contrast(wrapper_colour),
      background_colour: get_random_colour(num_from_name + 13, colours, range),
      wrapper_translate_x: wrapper_translate_x,
      wrapper_translate_y: wrapper_translate_y,
      wrapper_rotate: get_unit(num_from_name, 360),
      wrapper_scale: 1 + get_unit(num_from_name, @size / 12) / 10,
      is_mouth_open: get_boolean(num_from_name, 2),
      is_circle: get_boolean(num_from_name, 1),
      eye_spread: get_unit(num_from_name, 5),
      mouth_spread: get_unit(num_from_name, 3),
      face_rotate: get_unit(num_from_name, 10, 3),
      face_translate_x:
        if(wrapper_translate_x > @size / 6,
          do: wrapper_translate_x / 2,
          else: get_unit(num_from_name, 8, 1)
        ),
      face_translate_y:
        if(wrapper_translate_y > @size / 6,
          do: wrapper_translate_y / 2,
          else: get_unit(num_from_name, 7, 2)
        )
    }
  end

  def avatar(
        %BoringAvatar{
          name: name,
          colours: colours,
          title: title,
          size: size,
          square: square
        } = props
      ) do
    mask_id = unique_id(props)
    data = generate_data(name, colours)

    svg(
      viewBox: "0 0 #{@size} #{@size}",
      fill: "none",
      role: "img",
      xmlns: "http://www.w3.org/2000/svg",
      width: size,
      height: size
    ) do
      if(title, do: title([], title), else: "")

      mask(
        id: mask_id,
        maskUnits: "userSpaceOnUse",
        x: 0,
        y: 0,
        width: @size,
        height: @size
      ) do
        rect(width: @size, height: @size, rx: if(square, do: 0, else: @size * 2), fill: "#FFFFFF")
      end

      g(mask: "url(##{mask_id})") do
        rect(width: @size, height: @size, fill: data.background_colour)

        rect(
          x: 0,
          y: 0,
          width: @size,
          height: @size,
          transform:
            transform do
              translate(data.wrapper_translate_x, data.wrapper_translate_y)
              rotate(data.wrapper_rotate, @size / 2, @size / 2)
              scale(data.wrapper_scale)
            end,
          fill: data.wrapper_colour,
          rx: if(data.is_circle, do: @size, else: @size / 6)
        )

        g(
          transform:
            transform do
              translate(data.face_translate_x, data.face_translate_y)
              rotate(data.face_rotate, @size / 2, @size / 2)
            end
        ) do
          if data.is_mouth_open do
            path(
              d: "M15 #{19 + data.mouth_spread} c2 1 4 1 6 0",
              stroke: data.face_colour,
              fill: "none",
              stroke_linecap: "round"
            )
          else
            path(d: "M13,#{19 + data.mouth_spread} a1,0.75 0 0,0 10,0", fill: data.face_colour)
          end

          rect(
            x: 14 - data.eye_spread,
            y: 14,
            width: 1.5,
            height: 2,
            rx: 1,
            stroke: "none",
            fill: data.face_colour
          )

          rect(
            x: 20 + data.eye_spread,
            y: 14,
            width: 1.5,
            height: 2,
            rx: 1,
            stroke: "none",
            fill: data.face_colour
          )
        end
      end
    end
  end
end
